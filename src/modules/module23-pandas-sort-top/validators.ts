...
export const validateSortValues = (code: string, output: string): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasHighValue) {
    return { isValid: false, message: 'El output debe mostrar las cuentas más altas primero (la máxima es ~50.81)' };
  }
...
export const validateTop5 = (code: string, output: string): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
    return { isValid: false, message: 'No has escrito ningún código' };
  }
  ...
  const hasAscendingFalse = /ascending\s*=\s*False/.test(code);
  if (!hasAscendingFalse) {
    return { isValid: false, message: 'Usa ascending=False para ordenar de mayor a menor (las propinas más altas primero)' };
  }
  ...
  if (!hasTopTip) {
    return { isValid: false, message: 'El output debe mostrar las 5 propinas más altas (la máxima es 10.0)' };
  }
...
export const validateBottom5 = (code: string, output: string): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
    return { isValid: false, message: 'No has escrito ningún código' };
  }
  ...
  if (!hasTail) {
    return { isValid: false, message: 'Usa .tail(5) para obtener las 5 propinas más bajas (o sort descendente + .head(5))' };
  }
  ...
  if (!hasLowTip) {
    return { isValid: false, message: 'El output debe mostrar las 5 propinas más bajas (las menores están alrededor de 1.0)' };
  }
...
export const validateGroupbySort = (code: string, output: string): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
    return { isValid: false, message: 'No has escrito ningún código' };
  }
  ...
  if (!hasGroupby) {
    return { isValid: false, message: 'Usa .groupby() para calcular el promedio por grupo primero' };
  }
  ...
  const hasAscendingFalse = /ascending\s*=\s*False/.test(code);
  if (!hasAscendingFalse) {
    return { isValid: false, message: 'Ordena de mayor a menor con ascending=False para ver el día con mayor propina primero' };
  }
  ...
  const hasSunFirst = output.trimStart().startsWith('Sun') || /^\s*(day\s*\n)?\s*Sun/.test(output) || output.includes('Sun');
  if (!hasSunFirst) {
    return { isValid: false, message: 'El output debe mostrar los días ordenados por propina promedio (Sun tiene la mayor)' };
  }
