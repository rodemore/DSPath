...
export const validateApplyColumn = (
  code: string,
  output: string
  ...
  if (!hasNoAxis1) {
    return {
      isValid: false,
      message:
        'Para apply sobre una columna no uses axis=1. Aplica .apply() directamente sobre la Serie',
    };
  }
  ...
  if (!hasLetterGrade) {
    return {
      isValid: false,
      message: 'El output debe mostrar las notas convertidas a letra (A, B, C o F)',
    };
  }
...
export const validateApplyMultipleColumns = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasMultipleColumns) {
    return {
      isValid: false,
      message: 'Selecciona el bloque de columnas de notas con df[["col1", "col2", ...]]',
    };
  }
  ...
  if (!hasDecimalBetween0and1) {
    return {
      isValid: false,
      message: 'El output debe mostrar valores normalizados entre 0 y 1 para todas las materias',
    };
  }
...
export const validatePromedioClasificacion = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  const hasAprobado =
    output.includes('Aprobado') ||
    output.includes('aprobado') ||
    output.includes('Reprobado') ||
    output.includes('reprobado') ||
    output.includes('Sobresaliente') ||
    output.includes('sobresaliente');
  const hasNumeric = /\d{2}\.\d/.test(output);
  if (!hasAprobado && !hasNumeric) {
    return {
      isValid: false,
      message: 'El output debe mostrar el promedio o clasificación de cada estudiante',
    };
  }
...
export const validateRanking = (
  ...
  if (!hasAxis1) {
    return {
      isValid: false,
      message:
        'Usa axis=1 para procesar fila por fila (necesitas acceder a varias columnas de notas a la vez)',
    };
  }
  ...
  if (!hasMateriasColumns) {
    return {
      isValid: false,
      message: 'Usa las columnas de materias para determinar el perfil del estudiante',
    };
  }
  ...
  const hasProfile =
    output.includes('Ciencias') ||
    output.includes('ciencias') ||
    output.includes('Humanidades') ||
    output.includes('humanidades') ||
    output.includes('Equilibrado') ||
    output.includes('equilibrado');
  if (!hasProfile) {
    return {
      isValid: false,
      message:
        'El output debe mostrar el perfil de cada estudiante (Ciencias, Humanidades o Equilibrado)',
    };
  }
