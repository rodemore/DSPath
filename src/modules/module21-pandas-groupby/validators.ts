...
export const validateGroupbyOne = (
  code: string,
  output: string
  ...
  if (!hasDayInOutput) {
    return {
      isValid: false,
      message: 'El output debe mostrar los días con su promedio de total_bill',
    };
  }
...
export const validateGroupbyTwo = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasTwoColumns) {
    return {
      isValid: false,
      message:
        'Para agrupar por dos columnas, pásalas dentro de una lista: .groupby(["col1", "col2"])',
    };
  }
  ...
  if (!hasDayInOutput) {
    return {
      isValid: false,
      message: 'El output debe mostrar la combinación de día y fumador con el promedio de tip',
    };
  }
...
export const validateResetIndex = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasResetIndex && !hasAsIndex) {
    return {
      isValid: false,
      message:
        'Usa .reset_index() al final, o as_index=False dentro del groupby, para obtener un DataFrame normal',
    };
  }
  ...
  if (!hasDayInOutput) {
    return {
      isValid: false,
      message: 'El output debe mostrar los días como una columna normal del DataFrame',
    };
  }
  ...
  if (!hasDayAsColumn) {
    return {
      isValid: false,
      message: 'Con reset_index(), "day" debe aparecer como columna, no como índice',
    };
  }
