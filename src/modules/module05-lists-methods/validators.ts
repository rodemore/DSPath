...
export const validateAppendMethod = (
  code: string,
  output: string
  ...
  if (appendCount < 3) {
    return {
      isValid: false,
      message:
        'Debes usar .append() al menos 3 veces para agregar 3 colores. Ejemplo: colores.append("rojo")',
    };
  }
  ...
  if (!hasPrint) {
    return {
      isValid: false,
      message: 'Debes usar print() para mostrar la lista después de agregar los elementos',
    };
  }
  ...
  if (!outputClean.includes('[') || !outputClean.includes(']')) {
    return {
      isValid: false,
      message: 'El output debe mostrar una lista. Asegúrate de imprimir la lista completa',
    };
  }
...
export const validateExtendMethod = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasCorrectList) {
    return {
      isValid: false,
      message: 'Debes crear la lista numeros = [1, 2, 3] como indica el ejercicio',
    };
  }
  ...
  if (!hasExtend) {
    return {
      isValid: false,
      message:
        'Debes usar el método .extend() para agregar múltiples elementos. Ejemplo: numeros.extend([4, 5, 6])',
    };
  }
  ...
  if (!hasPrint) {
    return {
      isValid: false,
      message: 'Debes usar print() para mostrar la lista después de extenderla',
    };
  }
  ...
  if (!hasAllNumbers) {
    return {
      isValid: false,
      message: 'La lista debe contener los números del 1 al 6 después de usar extend([4, 5, 6])',
    };
  }
...
export const validateInsertAndPop = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasCorrectList) {
    return {
      isValid: false,
      message: 'Debes crear la lista letras = ["a", "b", "c"] como indica el ejercicio',
    };
  }
  ...
  if (!hasInsert) {
    return {
      isValid: false,
      message: 'Debes usar .insert(1, "x") para insertar "x" en la posición 1',
    };
  }
  ...
  if (!hasPop) {
    return {
      isValid: false,
      message: 'Debes usar el método .pop() para eliminar el último elemento',
    };
  }
  ...
  if (!hasPrint) {
    return {
      isValid: false,
      message: 'Debes usar print() para mostrar la lista después de las modificaciones',
    };
  }
  ...
  const hasCorrectResult =
    outputClean.includes('a') &&
    outputClean.includes('x') &&
    outputClean.includes('b') &&
    !outputClean.includes('c');
  if (!hasCorrectResult) {
    return {
      isValid: false,
      message:
        'Verifica el orden de las operaciones. Después de insertar "x" en posición 1 y hacer pop(), la lista debería ser ["a", "x", "b"]',
    };
  }
...
export const validateSumAndLen = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  const hasCalificaciones =
    /calificaciones\s*=\s*\[\s*85\s*,\s*90\s*,\s*78\s*,\s*92\s*,\s*88\s*\]/.test(code);
  if (!hasCalificaciones) {
    return {
      isValid: false,
      message:
        'Debes crear la lista calificaciones = [85, 90, 78, 92, 88] como indica el ejercicio',
    };
  }
  ...
  if (!hasSum) {
    return {
      isValid: false,
      message:
        'Debes usar la función sum() para sumar las calificaciones. Ejemplo: sum(calificaciones)',
    };
  }
  ...
  if (!hasLen) {
    return {
      isValid: false,
      message:
        'Debes usar la función len() para obtener la cantidad de calificaciones. Ejemplo: len(calificaciones)',
    };
  }
  ...
  if (!hasTotal) {
    return {
      isValid: false,
      message: 'Debes crear una variable llamada "total" con el resultado de sum(calificaciones)',
    };
  }
  ...
  if (!hasCantidad) {
    return {
      isValid: false,
      message:
        'Debes crear una variable llamada "cantidad" con el resultado de len(calificaciones)',
    };
  }
  ...
  if (!hasPromedio) {
    return {
      isValid: false,
      message: 'Debes crear una variable llamada "promedio" dividiendo total entre cantidad',
    };
  }
  ...
  if (isNaN(outputNum) || Math.abs(outputNum - 86.6) > 0.1) {
    return {
      isValid: false,
      message: 'El promedio de [85, 90, 78, 92, 88] debe ser 86.6. Verifica tus cálculos',
    };
  }
...
export const validateIndexAndCount = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  const hasCorrectList =
    /numeros\s*=\s*\[\s*5\s*,\s*10\s*,\s*15\s*,\s*10\s*,\s*20\s*,\s*10\s*,\s*25\s*\]/.test(code);
  if (!hasCorrectList) {
    return {
      isValid: false,
      message:
        'Debes crear la lista numeros = [5, 10, 15, 10, 20, 10, 25] como indica el ejercicio',
    };
  }
  ...
  if (!hasIndex) {
    return {
      isValid: false,
      message:
        'Debes usar .index(15) para encontrar la posición del número 15. Ejemplo: numeros.index(15)',
    };
  }
  ...
  if (!hasCount) {
    return {
      isValid: false,
      message:
        'Debes usar .count(10) para contar cuántas veces aparece el 10. Ejemplo: numeros.count(10)',
    };
  }
  ...
  if (!hasPosicion) {
    return {
      isValid: false,
      message: 'Debes crear una variable llamada "posicion" con el resultado de .index(15)',
    };
  }
  ...
  if (!hasApariciones) {
    return {
      isValid: false,
      message: 'Debes crear una variable llamada "apariciones" con el resultado de .count(10)',
    };
  }
  ...
  if (!outputStr.includes('2') || !outputStr.includes('3')) {
    return {
      isValid: false,
      message: 'El resultado debe mostrar que 15 está en la posición 2 y que 10 aparece 3 veces',
    };
  }
...
export const validateSortList = (
  ...
  // Verificar que crea la lista correcta
  const hasCorrectList =
    /precios\s*=\s*\[\s*45\s*,\s*23\s*,\s*67\s*,\s*12\s*,\s*89\s*,\s*34\s*\]/.test(code);
  if (!hasCorrectList) {
    return {
      isValid: false,
      message: 'Debes crear la lista precios = [45, 23, 67, 12, 89, 34] como indica el ejercicio',
    };
  }
  ...
  if (!hasSort) {
    return {
      isValid: false,
      message: 'Debes usar el método .sort() para ordenar la lista. Ejemplo: precios.sort()',
    };
  }
  ...
  if (!hasCorrectOrder) {
    return {
      isValid: false,
      message: 'La lista debe estar ordenada de menor a mayor: [12, 23, 34, 45, 67, 89]',
    };
  }
  ...
  if (index12 === -1 || index89 === -1 || index12 > index89) {
    return {
      isValid: false,
      message: 'La lista no está correctamente ordenada. Usa .sort() para ordenar de menor a mayor',
    };
  }
