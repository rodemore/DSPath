...
export const validateLoadDataFrame = (
  code: string,
  output: string
  ...
  if (!hasIrisData) {
    return {
      isValid: false,
      message:
        'El output debe mostrar las columnas del dataset iris. Verifica que estés imprimiendo df.head()',
    };
  }
...
export const validateShape = (
  ...
  if (!hasCorrectShape) {
    return {
      isValid: false,
      message: 'El output debe mostrar (150, 5), las dimensiones del dataset iris',
    };
  }
...
export const validateReadCsvWithSep = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasSepParameter) {
    return {
      isValid: false,
      message: 'Debes usar el parámetro sep="|" para especificar que el separador es pipe (|)',
    };
  }
  ...
  if (!hasTipsData) {
    return {
      isValid: false,
      message:
        'El output debe mostrar las columnas del dataset tips (total_bill, tip, etc.). Verifica que estés usando sep="|"',
    };
  }
...
export const validateSelectColumn = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasSpeciesColumn) {
    return {
      isValid: false,
      message: 'Debes seleccionar la columna "species" usando df["species"]',
    };
  }
  ...
  if (!hasEspecies) {
    return {
      isValid: false,
      message: 'Debes guardar la columna en una variable llamada "especies"',
    };
  }
  ...
  const hasSpeciesData =
    output.includes('setosa') || output.includes('versicolor') || output.includes('virginica');
  if (!hasSpeciesData) {
    return {
      isValid: false,
      message:
        'El output debe mostrar los valores de la columna species (setosa, versicolor, virginica)',
    };
  }
...
export const validateSelectMultipleColumns = (
  code: string,
  output: string
): { isValid: boolean; message?: string } => {
  if (!code.trim()) {
  ...
  if (!hasDoubleBracket) {
    return {
      isValid: false,
      message: 'Para seleccionar múltiples columnas debes usar doble corchete: df[[ ... ]]',
    };
  }
  ...
  if (!hasSepalLength || !hasSepalWidth) {
    return {
      isValid: false,
      message: 'Debes seleccionar las columnas "sepal_length" y "sepal_width"',
    };
  }
  ...
  if (!hasSepalos) {
    return {
      isValid: false,
      message: 'Debes guardar el resultado en una variable llamada "sepalos"',
    };
  }
  ...
  if (!hasCorrectColumns) {
    return {
      isValid: false,
      message: 'El output debe mostrar las columnas sepal_length y sepal_width',
    };
  }
...
export const validateIloc = (
  ...
  if (!hasCorrectRange) {
    return {
      isValid: false,
      message: 'Debes seleccionar las filas de la posición 5 a la 9 usando .iloc[5:10]',
    };
  }
  ...
  if (!hasSubset) {
    return {
      isValid: false,
      message: 'Debes guardar el resultado en una variable llamada "subset"',
    };
  }
...
export const validateLoc = (
  ...
  if (!hasVirginicas) {
    return {
      isValid: false,
      message: 'Debes guardar el resultado en una variable llamada "virginicas"',
    };
  }

  // Verificar que el output contiene solo virginica
  const hasOnlyVirginica =
    output.includes('virginica') && !output.includes('setosa') && !output.includes('versicolor');
  if (!hasOnlyVirginica) {
    return {
      isValid: false,
      message:
        'El output debe mostrar solo flores de la especie virginica. Verifica que el filtro esté correcto',
    };
  }
